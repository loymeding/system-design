...
ensure_kafka_topic() {
    ...
    echo "Creating topic: ${KAFKA_service_TOPIC}"
    ...
    docker exec kafka /usr/bin/kafka-topics \
        --bootstrap-server localhost:9092 \
        --list | grep -q "${KAFKA_service_TOPIC}" || {
        print_error "Topic was not created successfully"
        return 1
    }
    
    print_success "Topic ${KAFKA_service_TOPIC} created/verified"
    return 0
}
...
check_kafka_topics() {
    if ! docker exec kafka /usr/bin/kafka-topics \
        ...
        --list | grep -q "${KAFKA_service_TOPIC}"; then
        echo "Topic ${KAFKA_service_TOPIC} not found"
        return 1
    fi
    return 0
}
...
monitor_kafka_events() {
    ...
    print_step "Monitoring Kafka events for service: $service_id"
    ...
    while [ $counter -lt $max_wait ]; do
        ...
        if docker exec kafka /usr/bin/kafka-console-consumer \
            ...
            --topic "$KAFKA_service_TOPIC" \
            --from-beginning \
            --max-messages 1 | grep -q "$service_id"; then
            
            print_success "service event found in Kafka"
            return 0
        fi
    ...
    print_error "service event not found in Kafka after $max_wait seconds"
    return 1
}
...
check_required_vars() {
    ...
    export KAFKA_service_TOPIC="service-events"
...
cleanup_test_data() {
    ...
    if [ ! -z "$order_ID" ]; then
        docker exec mongodb mongosh "$MONGODB_URL" --quiet \
            --eval "db.services.deleteMany({order_id: '$order_ID'})" \
            > /dev/null 2>&1 || print_warning "Failed to cleanup MongoDB test services"
    fi
...
check_services_health() {
    ...
    for service in "user:8000" "order:8001" "service:8002"; do
        name=${service%:*}
        port=${service#*:}
...
# Тестирование service Service (CQRS)
