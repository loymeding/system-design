# order-management-system/services/utils/database.py
import os
import logging
import asyncio
...
class PostgresManager:
    ...
    def get_orders_by_owner(
        ...
        try:
            ...
            self._handle_db_error("get_orders_by_owner", e)

    def get_services_by_order(
        self, 
        model: Type[ModelType], 
        order_id: UUID
    ) -> List[ModelType]:
        ...
        try:
            with self.get_db() as db:
                return db.query(model).filter(model.order_id == order_id).all()
        except SQLAlchemyError as e:
            self._handle_db_error("get_services_by_order", e)

    def get_services_by_assignee(
        self, 
        model: Type[ModelType], 
        assignee_id: UUID
    ) -> List[ModelType]:
